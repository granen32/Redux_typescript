...
// types
import { TodostProps, ToggleTodos, IdTodos } from "../../@types/global";
...
export const todoSlice = createSlice({
  ...
  reducers: {
    // 투두 추가
    addTodo: (state, action: PayloadAction<TodostProps>) => {
      const todo = {
    ...
    // 투두스 완성
    toggleComplte: (state, action: PayloadAction<ToggleTodos>) => {
      const index = state.findIndex((todo) => todo.id === action.payload.id);
      state[index].completed = action.payload.completed;
    },
    delteTodo: (state, action: PayloadAction<IdTodos>) => {
